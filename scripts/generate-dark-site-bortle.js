...
const DARK_SITES = [
  ["Yellowstone National Park, Wyoming/Montana/Idaho, USA", "Yellowstone", 44.6, -110.5],
...
const LIGHT_POLLUTION_COLORMAP = [
  [8, 8, 8, 1], [16, 16, 16, 1.5], [24, 24, 24, 2], [32, 32, 32, 2.5], 
  [48, 48, 48, 3], [0, 0, 128, 3.5], [0, 0, 255, 4], [0, 128, 0, 4.5], 
  [0, 255, 0, 5], [128, 128, 0, 5.5], [255, 255, 0, 6], [255, 128, 0, 6.5],
  [255, 64, 0, 7], [255, 0, 0, 7.5], [255, 255, 255, 8], [192, 192, 192, 9],
];
...
function rgbToBortleScale(r, g, b) {
  ...
  for (const colorEntry of LIGHT_POLLUTION_COLORMAP) {
    ...
    const distance = Math.sqrt((r - mapR) ** 2 + (g - mapG) ** 2 + (b - mapB) ** 2);
...
async function findDarkestBortleInRadius(centerCoord, radiusKm, imageData, width, height) {
  const pixelsPerDegree = width / 360;
  const radiusDegrees = radiusKm / (111.32 * Math.cos((centerCoord.lat * Math.PI) / 180));
...
async function generateDarkSiteBortleMapping() {
  ...
  for (let i = 0; i < DARK_SITES.length; i++) {
    ...
    console.log(`[${i + 1}/${DARK_SITES.length}] Processing ${slug} (${fullName})...`);
    ...
    try {
      ...
      if (darkestBortle !== null) {
        ...
        console.log(`  → Darkest Bortle within 10km: ${Math.round(darkestBortle * 10) / 10}`);
      } else {
...
`;
  ...
  const outputPath = path.join(process.cwd(), 'src', 'data', 'darkSiteBortle.ts');
  ...
  const report = {
    ...
    statistics: {
      averageBortle: results.reduce((sum, r) => sum + r.darkestBortle, 0) / results.length,
  ...
  console.log(`   Average Bortle: ${Math.round(report.statistics.averageBortle * 10) / 10}`);
