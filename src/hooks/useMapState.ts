...
export function useMapState(config: MapStateConfig = {}) {
  ...
  const setConstrainedPan = useCallback((newPanX: number, newPanY: number) => {
    const constrained = constrainPan(newPanX, newPanY, zoom);
    setPanX(constrained.x);
    setPanY(constrained.y);
  }, [constrainPan, zoom]);

  const setConstrainedZoom = useCallback((newZoom: number | ((prev: number) => number)) => {
    setZoom(prevZoom => {
      const actualNewZoom = typeof newZoom === 'function' ? newZoom(prevZoom) : newZoom;
      const constrainedZoom = Math.max(finalConfig.minZoom, Math.min(finalConfig.maxZoom, actualNewZoom));
  ...
  }, [finalConfig.initialZoom, finalConfig.initialPanX, finalConfig.initialPanY]);
