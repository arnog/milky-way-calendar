  ...
  useEffect(() => {
    ...
    const calculateWeekData = async () => {
      ...
      for (let weekNumber = 1; weekNumber <= 52; weekNumber++) {
        // Calculate the start date of each week (Saturday start to match reference calendar)
        const startDate = new Date(year, 0, 1 + (weekNumber - 1) * 7 - 3)
        ...
        try {
          ...
          const optimalWindow = calculateOptimalViewingWindow(midWeek, location, gcData, moonData, twilightData)
          ...
          if (optimalWindow.startTime) {
            gcDataForRating = calculateGalacticCenterPosition(optimalWindow.startTime, location)
          }
          
          // Calculate visibility rating based on the actual optimal viewing window
          const visibility = calculateVisibilityRating(gcDataForRating, moonData, twilightData, optimalWindow)
          ...
          console.error(`Error calculating data for week ${weekNumber}:`, error)
