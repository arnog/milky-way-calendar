...
import type { Coordinate, DarkSiteResult, MultipleDarkSitesResult } from '../utils/lightPollutionMap';
...
export function useDarkSiteWorker(): UseDarkSiteWorkerReturn {
  ...
  const findNearestDarkSky = useCallback(async (
    startCoord: Coordinate,
    options: {
      maxDistance?: number;
      knownSites?: ReadonlyArray<SpecialLocation>;
    ...
    if (!isWorkerSupported) {
      ...
      const { findNearestDarkSky: fallbackFunction } = await import('../utils/lightPollutionMap');
  ...
  const findMultipleDarkSites = useCallback(async (
    startCoord: Coordinate,
    options: {
      maxDistance?: number;
      knownSites?: ReadonlyArray<SpecialLocation>;
    ...
    if (!isWorkerSupported) {
      // Fallback to main thread execution
      const { findMultipleDarkSites: fallbackFunction } = await import('../utils/lightPollutionMap');
      return fallbackFunction(
        startCoord,
        options.maxDistance,
        options.onProgress,
  ...
  const getBortleRatingForLocation = useCallback(async (
    coord: Coordinate
  ): Promise<number | null> => {
    if (!isWorkerSupported) {
      // Fallback to main thread execution
      const { getBortleRatingForLocation: fallbackFunction } = await import('../utils/lightPollutionMap');
      return fallbackFunction(coord);
    }

    return sendWorkerMessage<number | null>(
