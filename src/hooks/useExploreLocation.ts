...
export function useExploreLocation(): ExploreLocationHookType {
  const [location, setLocationState] = useState<Location | null>(() => {
    ...
    return storageService.getExploreLocation();
  });
  ...
  useEffect(() => {
    if (location) {
      ...
      storageService.setExploreLocationData(location, nearbyLocation?.matchedName || null);
    }
  }, [location]);
   ...
   */
  const updateLocation = useCallback((newLocation: Location, matchedName?: string | null) => {
    setLocationState(newLocation);
    ...
    storageService.setExploreLocationData(newLocation, finalMatchedName);
  }, []);
  ...
  const initializeFromHomeLocation = useCallback(() => {
    ...
    if (!location) {
      const homeLocation = storageService.getLocation();
      if (homeLocation) {
