...
export class MapImageCacheService {
  ...
  private inMemoryCache = new Map<string, { data: CachedImageData; timestamp: number }>();
  ...
  async loadImageAsBlob(
    ...
    const imagePath = await this.getBestImagePath(format, preferredSize, screenWidth);
