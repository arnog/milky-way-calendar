...
import { findMultipleDarkSites, MultipleDarkSitesResult, coordToNormalized } from "../utils/lightPollutionMap";
...
function ExplorePage({ isDarkroomMode: _isDarkroomMode }: ExplorePageProps) {
  ...
  const [darkSitesResult, setDarkSitesResult] = useState<MultipleDarkSitesResult | null>(null);
      ...
      (async () => {
        ...
        try {
          ...
          if (result) {
            ...
            setSearchError('No dark sky sites found within 500km of your location.');
          }
        } catch (error) {
  ...
  const handleUserLocationChangeWrapper = (location: Location, shouldClose = true) => {
  ...
  const handleGetDirections = (lat: number, lng: number) => {
    const googleMapsUrl = `https://www.google.com/maps/dir/?api=1&destination=${lat.toFixed(6)},${lng.toFixed(6)}`;
              ...
              <div className={exploreStyles.mapContainer}>
                ...
                <div className={exploreStyles.markersOverlay}>
                  {DARK_SITES.map((loc, idx) => {
                    ...
                    const { x: normalizedX, y: normalizedY } = coordToNormalized(lat, lng);
                  ...
                  {darkSitesResult && (
                    <>
                      ...
                      <div
                        className={exploreStyles.markerPosition}
                        style={{
                          left: `${coordToNormalized(darkSitesResult.primary.coordinate.lat, darkSitesResult.primary.coordinate.lng).x * 100}%`,
                          top: `${coordToNormalized(darkSitesResult.primary.coordinate.lat, darkSitesResult.primary.coordinate.lng).y * 100}%`
                        }}
                      >
                        <div
                          className={`${exploreStyles.darkSiteMarker} ${exploreStyles.nearestMarker}`}
                          onClick={() => handleDarkSiteClick(darkSitesResult.primary.coordinate.lat, darkSitesResult.primary.coordinate.lng)}
                          title={`Primary Dark Site (${darkSitesResult.primary.distance.toFixed(1)}km away, Bortle ${darkSitesResult.primary.bortleScale})`}
                        />
                      </div>
                      ...
                      {darkSitesResult.alternatives.map((alt, idx) => (
                        <div
                          ...
                          className={exploreStyles.markerPosition}
                          style={{
                            left: `${coordToNormalized(alt.coordinate.lat, alt.coordinate.lng).x * 100}%`,
                            top: `${coordToNormalized(alt.coordinate.lat, alt.coordinate.lng).y * 100}%`
                          }}
                        >
                          <div
                            className={`${exploreStyles.darkSiteMarker} ${exploreStyles.alternativeMarker}`}
                            onClick={() => handleDarkSiteClick(alt.coordinate.lat, alt.coordinate.lng)}
                            title={`Alternative Dark Site (${alt.distance.toFixed(1)}km away, Bortle ${alt.bortleScale})`}
                          />
                        </div>
                      ))}
                    </>
                ...
                {isSearching && (
                  <div className={exploreStyles.progressContainer}>
                    ...
                    <p className={exploreStyles.progressText}>
                      Analyzing light pollution map... {Math.round(searchProgress * 100)}%
                    </p>
                  </div>
                )}
                ...
                {darkSitesResult && (
                  <div className={exploreStyles.resultsWrapper}>
                    ...
                    <div className={exploreStyles.resultContainer}>
                      <h3 className={exploreStyles.resultTitle}>🎯 Primary Dark Site</h3>
                      <div className={exploreStyles.resultDetails}>
                        <p><strong>Distance:</strong> {darkSitesResult.primary.distance.toFixed(1)} km away</p>
                        <p><strong>Bortle Scale:</strong> {darkSitesResult.primary.bortleScale} (Excellent dark sky)</p>
                        <p><strong>Coordinates:</strong> {darkSitesResult.primary.coordinate.lat.toFixed(4)}, {darkSitesResult.primary.coordinate.lng.toFixed(4)}</p>
                      </div>
                      <div className={exploreStyles.resultButtons}>
                        ...
                        <button
                          onClick={() => handleGetDirections(darkSitesResult.primary.coordinate.lat, darkSitesResult.primary.coordinate.lng)}
                      ...
                      {darkSitesResult.primary.nearestKnownSite && (
                        <div className={exploreStyles.knownSiteSection}>
                          ...
                          <p className={exploreStyles.knownSiteDescription}>
                            <strong>{darkSitesResult.primary.nearestKnownSite.fullName}</strong> is {darkSitesResult.primary.nearestKnownSite.distance.toFixed(1)}km from this dark site.
                            This location may offer better road access and facilities.
                          </p>
                          <div className={exploreStyles.knownSiteButtons}>
                            <button
                              onClick={() => handleKnownSiteClick(darkSitesResult.primary.nearestKnownSite!.name)}
                              className={exploreStyles.knownSiteButton}
                            >
                              View {darkSitesResult.primary.nearestKnownSite.name}
                            </button>
                            <button
                              onClick={() => handleGetDirections(darkSitesResult.primary.nearestKnownSite!.coordinate.lat, darkSitesResult.primary.nearestKnownSite!.coordinate.lng)}
                              className={exploreStyles.knownSiteDirectionsButton}
                            >
                              Get Directions
                            </button>
                          </div>
                    ...
                    {darkSitesResult.alternatives.length > 0 && (
                      <div className={exploreStyles.alternativesSection}>
                        <h3 className={exploreStyles.alternativesTitle}>🌟 Alternative Dark Sites</h3>
                        <p className={exploreStyles.alternativesDescription}>
                          {darkSitesResult.alternatives.length} additional excellent locations in different directions.
                        </p>
                        
                        <div className={exploreStyles.alternativesGrid}>
                          {darkSitesResult.alternatives.map((alt, idx) => (
                            <div key={idx} className={exploreStyles.alternativeCard}>
                              <div className={exploreStyles.alternativeCardHeader}>
                                <span className={exploreStyles.alternativeNumber}>#{idx + 1}</span>
                                <div className={exploreStyles.alternativeMetrics}>
                                  <span className={exploreStyles.distanceMetric}>{alt.distance.toFixed(0)}km</span>
                                  <span className={exploreStyles.bortleMetric}>B{alt.bortleScale}</span>
                                </div>
                              </div>
                              
                              <div className={exploreStyles.alternativeCoords}>
                                {alt.coordinate.lat.toFixed(3)}, {alt.coordinate.lng.toFixed(3)}
                              </div>
                              
                              <div className={exploreStyles.alternativeActions}>
                                <button
                                  onClick={() => handleDarkSiteClick(alt.coordinate.lat, alt.coordinate.lng)}
                                  className={exploreStyles.alternativeActionButton}
                                  title="View Milky Way visibility data"
                                >
                                  🔭 View
                                </button>
                                <button
                                  onClick={() => handleGetDirections(alt.coordinate.lat, alt.coordinate.lng)}
                                  className={exploreStyles.alternativeActionButton}
                                ...
                                {alt.nearestKnownSite && (
                                  <button
                                    onClick={() => handleKnownSiteClick(alt.nearestKnownSite!.name)}
                                    className={exploreStyles.alternativeActionButton}
                                    title={`Nearest accessible: ${alt.nearestKnownSite.fullName} (${alt.nearestKnownSite.distance.toFixed(0)}km)`}
                                  >
                    ...
                    <div className={exploreStyles.resultsMapSection}>
                      <h3 className={exploreStyles.resultsMapTitle}>🗺️ Found Locations Map</h3>
                      <p className={exploreStyles.resultsMapDescription}>
                        Your location (green) and all found dark sites. Red = primary, orange = alternatives.
                      </p>
