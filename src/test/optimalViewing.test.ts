...
describe("optimalViewing", () => {
  describe("getOptimalViewingWindow", () => {
    ...
    it("should return empty window when GC is not visible", () => {
      ...
      const result = getOptimalViewingWindow(
        gcData,
        moonData,
        twilightData,
        mockLocation,
    ...
    it("should calculate window when GC rises after dark", () => {
      ...
      expect(result.startTime?.getTime()).toBe(
        new Date("2024-07-15T23:00:00Z").getTime(),
      );
      // End time should be when GC sets
      expect(result.endTime?.getTime()).toBe(
        new Date("2024-07-16T03:00:00Z").getTime(),
      );
    });
