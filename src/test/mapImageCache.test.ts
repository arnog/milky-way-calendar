...
describe("MapImageCacheService", () => {
  ...
  describe("Configuration", () => {
    ...
    it("should include both grayscale and color image options", () => {
      const grayscaleOptions = CACHE_CONFIG.IMAGE_OPTIONS.filter(
        (opt) => opt.format === "grayscale",
      );
      const colorOptions = CACHE_CONFIG.IMAGE_OPTIONS.filter(
        (opt) => opt.format === "color",
      );
    ...
    it("should have grayscale as highest priority for calculations", () => {
      const grayscaleOption = CACHE_CONFIG.IMAGE_OPTIONS.find(
        (opt) => opt.format === "grayscale",
      );
  ...
  describe("loadImageAsData", () => {
    it("should load image and return ImageData", async () => {
      ...
      ).mockResolvedValueOnce(mockImageBitmap);
      mockCache.match.mockResolvedValue(null);
  ...
  describe("clearCache", () => {
    it("should clear all cached images", async () => {
      ...
      expect(mockCache.delete).toHaveBeenCalledTimes(
        CACHE_CONFIG.IMAGE_OPTIONS.length,
      );
    });

    it("should handle cache API errors gracefully", async () => {
      mockCaches.open.mockRejectedValueOnce(
