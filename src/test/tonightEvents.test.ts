...
describe("calculateTonightEvents", () => {
  it("should return events in chronological order", () => {
    ...
    if (events.nightStart) eventTimes.push({ name: "Night Start", time: events.nightStart });
    if (events.moonRise) eventTimes.push({ name: "Moonrise", time: events.moonRise });
    if (events.moonSet) eventTimes.push({ name: "Moonset", time: events.moonSet });
    if (events.gcRise) eventTimes.push({ name: "GC Rise", time: events.gcRise });
    if (events.gcTransit) eventTimes.push({ name: "GC Transit", time: events.gcTransit });
    if (events.gcSet) eventTimes.push({ name: "GC Set", time: events.gcSet });
    if (events.nightEnd) eventTimes.push({ name: "Night End", time: events.nightEnd });
    if (events.sunRise) eventTimes.push({ name: "Sunrise", time: events.sunRise });
    ...
    if (events.gcRise && events.gcTransit && events.gcSet) {
      ...
      expect(events.gcTransit.getTime()).toBeGreaterThan(events.gcRise.getTime());
    ...
    if (events.sunSet && events.nightStart) {
      ...
      expect(events.nightStart.getTime()).toBeGreaterThan(events.sunSet.getTime());
    }
  ...
  it("should use 6pm as fulcrum point", () => {
    ...
    if (events.sunSet) {
      const hoursFromSixPm = Math.abs(events.sunSet.getTime() - sixPm.getTime()) / (60 * 60 * 1000);
