...
function getTimezoneOffsetMinutes(ianaTimezone: string, date: Date): number {
  try {
    ...
    const targetTime = new Date(localTime.toLocaleString('en-US', { timeZone: ianaTimezone }));
 ...
 */
export function formatDateForStructuredData(date: Date, location: Location): string {
  try {
    const timezoneInfo = getLocationTimezone(location);
    
    // Format the date in the location's timezone
    const year = date.toLocaleString('en-US', { timeZone: timezoneInfo.iana, year: 'numeric' });
    const month = date.toLocaleString('en-US', { timeZone: timezoneInfo.iana, month: '2-digit' });
    const day = date.toLocaleString('en-US', { timeZone: timezoneInfo.iana, day: '2-digit' });
    const hour = date.toLocaleString('en-US', { timeZone: timezoneInfo.iana, hour: '2-digit', hour12: false });
    const minute = date.toLocaleString('en-US', { timeZone: timezoneInfo.iana, minute: '2-digit' });
    const second = date.toLocaleString('en-US', { timeZone: timezoneInfo.iana, second: '2-digit' });
