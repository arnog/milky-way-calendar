...
export default function ClockFace({
  ...
  const getHighlightedArcs = (
    eventId: string | null,
  ): { sun: boolean; moon: boolean; gc: boolean } => {
    ...
    >
      ...
      <g className={styles.hourMarkers}>
        {hourMarkerConfig.angles.map((angle, index) => {
          const pos = getClockLabelPosition(
            angle,
            hourMarkerConfig.textRadius,
            centerX,
            centerY,
          );
          ...
          return (
            <g key={angle}>
              ...
              <line
                x1={
                  centerX +
                  hourMarkerConfig.lineOuterRadius * Math.sin(angleRad)
                }
                y1={
                  centerY -
                  hourMarkerConfig.lineOuterRadius * Math.cos(angleRad)
                }
                x2={
                  centerX +
                  hourMarkerConfig.lineInnerRadius * Math.sin(angleRad)
                }
                y2={
                  centerY -
                  hourMarkerConfig.lineInnerRadius * Math.cos(angleRad)
                }
      ...
      >
        {sunArcs.map((arc, index) => (
          <path
            ...
            strokeWidth={
              highlightedArcs.sun
                ? ARC_CONFIG.DEFAULT_STROKE_WIDTH + 5
                : ARC_CONFIG.DEFAULT_STROKE_WIDTH
            }
            fill="none"
      ...
      >
        {gcArcs.map((arc, index) => (
          <path
            ...
            strokeWidth={
              highlightedArcs.gc
                ? (arc.strokeWidth || ARC_CONFIG.DEFAULT_STROKE_WIDTH) + 5
                : arc.strokeWidth || ARC_CONFIG.DEFAULT_STROKE_WIDTH
            }
            fill="none"
            className={arc.className}
            opacity={
              highlightedArcs.gc
                ? Math.max(0.9, arc.opacity || 1)
                : arc.opacity || 1
            }
            aria-label={`${arc.className?.includes("optimal") ? "Optimal viewing" : "Galactic Center visible"} period`}
            style={{
              filter: highlightedArcs.gc
                ? "drop-shadow(0 0 8px currentColor)"
                : "none",
              transition: "all 0.2s ease",
            }}
      ...
      {showClockHand && (
        ...
        >
          <line
            x1={
              centerX -
              dimensions.radiusStep *
                Math.sin((currentTimeAngle * Math.PI) / 180)
            }
            y1={
              centerY +
              dimensions.radiusStep *
                Math.cos((currentTimeAngle * Math.PI) / 180)
            }
            x2={
              centerX +
              hourMarkerConfig.textRadius *
      ...
      <g className={styles.moonPhaseCenter}>
        ...
        >
          <div className={styles.moonPhaseCenterContent}>
            <Icon name={getMoonPhaseIcon(moonPhase, location.lat)} size="md" />
      ...
      <g className={styles.eventLabels}>
        {clockEvents.map((event, index) => {
          ...
          const labelPos = getClockLabelPosition(
            event.angle,
            effectiveRadius,
            centerX,
            centerY,
                ...
                <Tooltip content={event.tooltip} placement="top">
                  ...
                  >
                    ...
                    {event.icons && event.icons.length > 1 ? (
                      ...
                      <Icon name={event.icon} size="md" aria-hidden="true" />
                    )}
                    <FormattedTime date={event.time} aria-hidden="true" />
                  </button>
                </Tooltip>
              </foreignObject>
            </g>
